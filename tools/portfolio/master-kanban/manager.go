...
func runManagerSweep(p *pgxpool.Pool) error {
	...
	for _, init := range initiatives {
		...
		engaged, err := isLowerLayerEngaged(ctx, p, init.id, beadRefs, vkRefs)
		if err == nil && engaged {
			signal.HasActiveWork = true
		}
...
func isLowerLayerEngaged(ctx context.Context, p *pgxpool.Pool, initID string, beadRefs []string, vkRefs []string) (bool, error) {
