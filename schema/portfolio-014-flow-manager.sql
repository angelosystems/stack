-- portfolio-014-flow-manager.sql — Kanban Flow Manager Event Kind
--
-- Erweitert initiative_event um 'flow_action' Kind
-- und 'flow_manager' Source-Backend für die persistierten Fluss-Diagnosen und -Vorschläge.
--
...
ALTER TABLE portfolio.initiative_event ADD CONSTRAINT initiative_event_kind_check
  CHECK (kind = ANY (ARRAY[
    ...
    'deployed', 'workspace_started', 'ai_message', 'ai_action', 'sage_action', 'flow_action'
  ]));
...
ALTER TABLE portfolio.initiative_event ADD CONSTRAINT initiative_event_source_backend_check
  CHECK (source_backend = ANY (ARRAY[
    'vk', 'solartown', 'github', 'plan_file', 'master', 'coder', 'sage', 'flow_manager'
  ]));
