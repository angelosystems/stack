...
| D3 | ~~Twenty-Login ohne Perimeter~~ **REVIDIERT (Mario 07-14): Authentik-ForwardAuth vor der UI (Gruppe „authentik Admins") + Google-OAuth als einziger Twenty-Login (Passwort-Login wird nach Google-Verifikation abgeschaltet).** API-Pfade `/rest`,`/graphql`,`/metadata` bleiben vom Perimeter ausgenommen (eigene Bearer-Auth, anonym 403) — sonst wären ActivePieces/Gaia ausgesperrt. Enterprise-OIDC weiterhin bewusst nicht. |
